...
constexpr char sqrtc = 'S';
const string sqrtkey = "sqrt";
constexpr char powc = 'P';
const string powkey = "pow";
constexpr char quitc = 'q';
	...
	switch (ch)
	{
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
    			...
    			cin.putback(ch);
    			if (s == quit) return Token{quitc};
    			if (s == declkey) return Token{let};
    			if (s == sqrtkey) return Token{sqrtc};
    			if (s == powkey) return Token{powc};
...
double sqrtd()
{
    ...
    cin.putback(ch);
    double d = primary();
    if (d < 0) error("sqrt: Can't be a negative value");
    return sqrt(d);
}
